...
/// Reactive list
class StatefulList<E> extends DelegatingList<E> implements List<E> {
  ...
  StatefulList() : super(<E>[]) {
    _onChange = _changes.stream.asBroadcastStream();
  }

  StatefulList.filled(int length, E fill, {bool growable = false})
      : super(List<E>.filled(length, fill, growable: growable)) {
    _onChange = _changes.stream.asBroadcastStream();
  }

  StatefulList.from(Iterable<E> elements, {bool growable = true})
      : super(List<E>.from(elements, growable: growable)) {
    _onChange = _changes.stream.asBroadcastStream();
  }

  StatefulList.of(Iterable<E> elements, {bool growable = true})
      : super(List<E>.of(elements, growable: growable));

  StatefulList.generate(
    int length,
...
class BoundList<S, E> extends StatefulList<E> {
  final StatefulList<S> binding;
...
/// A record of change in a [StatefulList]
